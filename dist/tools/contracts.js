...
export function registerContractTools(server, config) {
    server.tool("self_get_contract_addresses", "Get all deployed Self Protocol contract addresses for a given network", { network: z.enum(["mainnet", "testnet"]).optional() }, async ({ network }) => {
        const net = network ?? config.network;
        const addrs = ADDRESSES[net];
        if (!addrs)
            return formatError(`Unknown network: ${net}`);
    ...
    server.tool("self_check_verification", "Check if a merkle root is valid in a Self Protocol identity registry", {
        attestationId: z
            .number()
        ...
        try {
            ...
            const attId = "0x" + attestationId.toString(16).padStart(64, "0");
            const registryAddress = await hub.registry(attId);
            if (registryAddress === "0x0000000000000000000000000000000000000000") {
                return formatError(`No registry found for attestationId ${attestationId} on ${net}`);
            }
            const registry = new ethers.Contract(registryAddress, REGISTRY_ABI, provider);
            const valid = await registry.checkIdentityCommitmentRoot(merkleRoot);
            return formatSuccess({
                network: net,
                attestationId,
                registryAddress,
    ...
    server.tool("self_get_registry_info", "Query identity registry state (merkle root, OFAC roots) for a document type", {
        attestationId: z
            .number()
            .describe("1=passport, 2=ID card, 3=aadhaar, 4=kyc"),
        network: z.enum(["mainnet", "testnet"]).optional(),
        ...
        try {
            ...
            const registry = new ethers.Contract(registryAddress, REGISTRY_ABI, provider);
            const [merkleRoot, passportNoOfacRoot, nameAndDobOfacRoot, nameAndYobOfacRoot] = await Promise.all([
