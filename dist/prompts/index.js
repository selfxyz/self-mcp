...
export function registerPrompts(server) {
    server.prompt("self_integrate_sdk", "Step-by-step guide to adding Self identity verification to your app", {
        platform: z
...
Store verification configs keyed by configId. The SDK calls getActionId to map user+data to a config, then getConfig to retrieve it.`,
    ...
    server.prompt("self_deploy_verifier", "Guide to building and deploying a custom SelfVerificationRoot contract on Celo", {
        features: z
            .string()
            .optional()
            .describe("Comma-separated features: age_check, ofac, nationality, birthday, token_gate"),
    }, async ({ features }) => {
...
- KYC = bytes32(4)`;
    ...
    server.prompt("self_verify_proof_backend", "Guide to setting up server-side proof verification with @selfxyz/core", {
        framework: z
            .string()
            .optional()
